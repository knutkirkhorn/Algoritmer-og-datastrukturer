...
public class LempelZivKompresjon {
    ...
    private int finnBesteMatch(ArrayList<Byte> bytesToMatch, int startIndex) {
        ...
        //Sjekke kor langt en kan gå tilbake å søke etter lignenede bytes
        ...
        for (int i = startBackIndex; i <= (distanceBack - MINIMUMS_ORDLENGDE); i++) { //Går for hver bokstav siden -127 eller så langt som mulig fra startVerdi og opp til start-verdi
            ...
            for (int j = i, k = 0; k < bytesToMatch.size(); j++, k++) { //Sjekker for hvert ord
                if (bytesFromFile[j] != bytesToMatch.get(k)) { //Sjekker hver bokstav i hvert ord
                    isFound = false;
                    break;
                }
            }
    ...
    public void komprimerFil() {
        ...
        int byteIndex = 0; //Kor en skal sette inn neste byte [-10], [10, 10] som beskrivet i leksjon
        ...
        for (int i = 0; i < bytesFromFile.length; i++) { //For hver bokstav i hele filen
            ...
            for (int j = i; j < bytesFromFile.length; j++) { //For i, så går den i en løkke frammover slik at vi får ulike ord: ABCDEF blir til ABC (i=0), og så BCD (i=1) osv...
                ...
                if ((currentBytes.size() >= MINIMUMS_ORDLENGDE) && (i >= MINIMUMS_ORDLENGDE) && (bytesFromFile.length - i >= MINIMUMS_ORDLENGDE)) { //TODO: (bytesFromFile.length - (i + 1) > MINIMUMS_ORDLENGDE)
    ...
    public static void main(String[] args) {
        LempelZivKompresjon lempelZivNy = new LempelZivKompresjon("TestFil.txt", "TestFilKomprimert.ingknu");
